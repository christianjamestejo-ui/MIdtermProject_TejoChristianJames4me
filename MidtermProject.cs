...
namespace MidtermProject
{
    ...
    {
        new Subject { Code="IT-101A", Title="Introduction to Computing"},
        new Subject { Code="IT-102A", Title="Computer Programming"},
        new Subject { Code="IT-103A", Title="Introduction to Human Computer Interactions"},
        new Subject { Code="MATH-105A", Title="Discrete Mathematics"},
        new Subject { Code="GEC-101A", Title="Mathematics in the Modern World"},
        new Subject { Code="GEC-102A", Title="Philippine Popular Culture"},
        new Subject { Code="PE-101A", Title="Physical Education 1"},
        new Subject { Code="THEO-101A", Title="Introduction to Catholic Faith 1"},
        new Subject { Code="COMP-102IT", Title="Computer Applications"},
        new Subject { Code="IT-104A", Title="Computer Programming 2"},
        new Subject { Code="IT-105A", Title="Platform Technologies"},
        new Subject { Code="IT-106A", Title="IT Social and Professional Issues"},
        new Subject { Code="GEC-103A", Title="Understanding the Self"},
        new Subject { Code="GEC-104A", Title="Art Appreciation"},
        new Subject { Code="RZL-101A", Title="Life and Works of Rizal"},
        new Subject { Code="PE-102A", Title="Physical Education"},
        new Subject { Code="THEO-102A", Title="Scriptures, the Sacraments and Liturgy"}
    };
    }
    ...
    public class EnrollStudent
    {
        public void Execute(List<Student> students)
        {
            ...
            enrolled.FirstName = selected.FirstName;
            enrolled.LastName = selected.LastName;
            ...
            {
                Console.WriteLine(SubjectList.Subjects[i].Code + " - " + SubjectList.Subjects[i].Title);
            }

            Console.WriteLine("\nEnter 9 Subject Codes:\n");
            ...
            while (enrolled.Subjects.Count < 9)
            {
                ...
                for (int i = 0; i < SubjectList.Subjects.Count; i++)
                {
                    if (SubjectList.Subjects[i].Code == code)
                    {
                        validSubject = true;
                        break;
                    }
                ...
                if (!validSubject)
                {
                    Console.WriteLine("Invalid subject code. Please choose from the list.");
                    continue;
                }

                bool duplicate = false;

                for (int i = 0; i < enrolled.Subjects.Count; i++)
                {
                    if (enrolled.Subjects[i] == code)
                    {
                        duplicate = true;
                        break;
                    }
            ...
            {
                List<string> existing = new List<string>(File.ReadAllLines("EnrolledStudents.json"));

                for (int i = 0; i < existing.Count; i++)
                {
                    jsonLines.Add(existing[i]);
                }
            }
    ...
    public class GradeStudent
    {
        public void Execute(List<Student> students)
        {
            ...
            List<EnrolledStudent> enrolled = new List<EnrolledStudent>();

            foreach (string line in File.ReadAllLines(path))
            {
                enrolled.Add(JsonSerializer.Deserialize<EnrolledStudent>(line));
            }

            for (int i = 0; i < enrolled.Count; i++)
            {
                Console.WriteLine($"{i + 1}. {enrolled[i].LastName}, {enrolled[i].FirstName}");
            }

            int choice = int.Parse(Console.ReadLine()) - 1;

            var student = enrolled[choice];
            ...
            output += "++++++++++++++++++++\n";
            output += $" {student.Course} - {student.Year} \n";
            output += "++++++++++++++++++++\n";

            for (int i = 0; i < student.Subjects.Count; i++)
            {
                output += $" {student.Subjects[i]} ----------{grades[i]} \n";
            }

            output += "++++++++++++++++++++\n";

            File.WriteAllText("Names.txt", output);
